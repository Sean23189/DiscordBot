...
@plugin.command()
@lightbulb.command('showall', 'Show all entries from the userinput list!')
@lightbulb.implements(lightbulb.SlashCommand)
async def showall(ctx):
    # Open the userinput.txt file in read mode
    with open('userinput.txt', 'r', encoding='utf-8') as file:
        # Create a new embed
        embed = hikari.Embed(
            title='All Entries to the userinput file!',
            color=hikari.Color.from_rgb(255, 0, 0)
        )
        # Read each line and add it as a field to the embed
        line_num = 0
        for line in file:
            line_num += 1
            embed.add_field(name=('Entry N.' + str(line_num)), value=line.strip())
    # Send the embed
